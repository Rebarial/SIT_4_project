...
''')
print(cursor.fetchall())
#Запрос на добавление в таблицу работу с клиентом и всех этапов в таблицу похождения этапов
prod = 2
cli = 3
manager = 2
cursor.execute('''
...
''', {"p_prod": prod, "p_manager": manager, "p_client": cli})
cursor.execute('''
Insert into
pass_stage
(work_id, stage_id)
select work_id, stage_id from work_with_client join stage where 
product_id = :p_prod and manager_id = :p_manager and client_id = :p_client
''', {"p_prod": prod, "p_manager": manager, "p_client": cli})
#con.comit()

cursor.execute('''
select  * from work_with_client
''')

print(cursor.fetchall())
cursor.execute('''
select * from pass_stage
''')
print(cursor.fetchall())
#Обновление даты на текущее в прошедших этапов с указанием work_id и stage_id
cursor.execute('''
update pass_stage set date = datetime('now','localtime') where work_id = :p_work and stage_id = :p_stage
''', {"p_work": 12, "p_stage": 2})
#con.comit()
